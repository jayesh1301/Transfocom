...
export default function EditCosting1() {
 ...
 const[costingname,setCostingname]=useState('')
  ...
  if (!oltctext){
    Swal.fire({
      icon: "warning", // 'warning' should be lowercase
      title: "OLTC is mandatory!",
      confirmButtonText: "OK",
      confirmButtonColor: "red",
      iconHtml: "", // this is not needed for the 'warning' icon
      animation: true // should be a boolean value
    });
    setIsLoading(false);
    return
  }
    try {
      ...
      const capitalizedCostingName = costingname ? costingname.toUpperCase().replace(/\s/g, "") : null;
      const sanitizedCostingName = capitalizedCostingName ? capitalizedCostingName.replace(/\s+/g, " ").replace(/\n/g, "") : null;
  ...
  const handleCostingNameChange = (event) => {
    ...
    console.log(capitalizedCostingName)
    handleChange({
      target: {
        name: "costingname",
  ...
  return (
    ...
    {isLoading ? (
      ...
      <Paper elevation={6} style={{ position: "relative", marginTop: 20,padding:'14px' }}>
        <Box
          sx={{
        ...
        >
          <Box component="form" noValidate sx={{ mt: 1 }}>
            ...
            >
              ...
              <TextField
  style={{ width: "570px", margin: "0 0px" }}
  fullWidth
  id="costingName"
  label="Costing Name"
  name="costingname"
  autoComplete="CostingName"
  value={values.costingname !== undefined ? values.costingname : costingname}
